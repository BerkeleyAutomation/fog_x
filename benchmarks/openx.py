...
def check_and_download_trajectory(exp_dir, dataset_name, trajectory_index):
    ...
    local_file_pattern = LOCAL_FILE_TEMPLATE.format(exp_dir=exp_dir, dataset_name=dataset_name, index=trajectory_index)
    if not any(os.path.exists(file) for file in glob.glob(local_file_pattern)):
        data_url = DATA_URL_TEMPLATE.format(dataset_name=dataset_name, index=trajectory_index)
        subprocess.run(["gsutil", "-m", "cp", data_url, dataset_dir], check=True)
    else:
        print(f"Trajectory {trajectory_index} of dataset {dataset_name} already exists in {dataset_dir}. Skipping download.")
    ...
    dataset_info_json_url = DATASET_INFO_JSON_URL_TEMPLATE.format(dataset_name=dataset_name)
    ...
    with ThreadPoolExecutor() as executor:
        ...
        for dataset_name in dataset_names:
            for i in range(num_trajectories):
                futures.append(executor.submit(check_and_download_trajectory, exp_dir, dataset_name, i))
...
def main():
    ...
    for dataset_name in args.dataset_names:
        ...
        rlds_loading_time, num_loaded_rlds = measure_loading_time(RLDSLoader, dataset_dir, args.num_trajectories)
